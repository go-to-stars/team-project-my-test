...
import { reedBookID } from './authorization';
...
function showMessageIfEmpty() {
  if (
    JSON.parse(localStorage.getItem('id')).length > 0 &&
    reedBookID().length > 0
  ) {
...
function fetchArrayOfBookId() {
  return (arrayOfBooksId = reedBookID());
}
fetchArrayOfBookId();
...
function refreshTotalPages() {
  totalPages = Math.ceil(reedBookID().length / paginator);
  return totalPages;
}
// function refreshTotalPages() {
...
function makeArrayToPaginate(page, paginator) {
  ...
  const arrayOfBooksToPaginate = reedBookID().splice(
    (page - 1) * paginator,
    paginator
  );
...
function paginate(event) {
  ...
  if (reedBookID().length <= paginator) {
    return;
  }
